...
final class HostGate
{
    ...
    public static function setupGuide(string $hostName, ?string $via = 'ftp'): array
    {
        ...
        return [
            ...
            '',
            '1. Mint your deploy token and upload the host file:',
            '   php pinoox pinroll:token {your-label}',
            '   → storage/pinroll/tokens/{your-label}.php on the host (cPanel/SFTP)',
            '   Plain token stays in .pinoox/pinroll.config.php locally.',
            '',
            '2. Install pingate.php once (if not already on the host):',
            '   php pinoox pinroll:gate ' . $hostName,
            '',
            '3. Export platform vendor when needed:',
            '   php pinoox pinroll:vendor --push',
            '',
            '4. Push + install — or go live in one step:',
            '   php pinoox pinroll:deploy ' . $hostName . ' --via=pinion',
        ];
    }
