...
#[AsCommand(
    name: 'pinroll:push',
    description: 'Build apps with pinx and push to a target',
    aliases: ['pinroll:deploy', 'pinroll:prod'],
)]
class PinrollPushCommand extends Terminal
{
    protected function configure(): void
    {
        $this
            ...
            ->addOption('apply', 'a', InputOption::VALUE_NONE, 'Apply release on target after push')
    ...
    protected function execute(InputInterface $input, OutputInterface $output): int
    {
        ...
        try {
            ...
            if (
                ...
                && \Pinoox\Pinroll\Target\TargetGate::isConfigured(Pinroll::targets()->raw($targetName))
            ) {
                $shouldApply = $io->confirm('Apply release on host via PinGate after upload?', false);
            }
    ...
    private function printHeader(
        ...
        $io->definitionList(
            ...
            ['Apply' => $apply ? '<fg=green>yes</> <fg=gray>(-a)</>' : '<fg=gray>no</>'],
        );
        $io->newLine();
    }
}
