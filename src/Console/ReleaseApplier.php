...
final class ReleaseApplier
{
    ...
    public function applyOnTarget(
        ...
        throw new PinrollException(
            implode("\n", TargetGate::setupGuide((string) ($resolvedTarget['name'] ?? 'production'))),
        );
    }
    ...
    private function applyViaPinGate(string $gateUrl, string $token, string $deployId, RolloutSession $session): void
    {
        $label = $deployId !== '' ? $deployId : 'latest';
        PushProgress::arrow('PinGate apply: ' . $label);
        $result = PinGateClient::apply($gateUrl, $token, $deployId);
        $status = (string) ($result['status'] ?? 'applied');
        $appliedId = (string) ($result['deploy_id'] ?? $deployId);
        $session->addStep(
            'apply',
            'ok',
            'Applied via PinGate (' . $status . ')' . ($appliedId !== '' ? ': ' . $appliedId : ''),
        );
    }
    ...
    private function applyViaSsh(array $ssh, array $rawTarget, string $deployId, RolloutSession $session): void
    {
        ...
        $remote = 'cd ' . escapeshellarg($cd) . ' && php pinoox pinroll:apply --local'
